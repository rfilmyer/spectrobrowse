...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let spectrograms = waveforms
        .par_iter()
        .map(move |(filepath, waveform)| {
            (
                filepath.to_owned(),
                lib::compute_spectrogram(waveform.to_owned(), window_size, overlap, true),
            )
        })
        .filter_map(|(filepath, spectrogram)| match spectrogram {
    ...
    spectrograms.for_each(move |(audio_filepath, spectrogram)| {
        let output_path = std::path::Path::new(output_path).join(
            audio_filepath
                .with_extension("png")
                .file_name()
        ...
        let image_width = max(
            image_height,
            (f32::sqrt(image_height as f32) * f32::sqrt(num_samples as f32)).round() as usize,
        );
        println!(
            "Generating a {} wide x {} high image at {}",
            image_width,
            image_height,
